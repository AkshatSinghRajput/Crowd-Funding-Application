...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      mailsend(userData.email, vericationEmailLink);
      success = true;
      res.json({ success, authtoken, emailCode });
    } catch (error) {
      success = false;
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (!user) {
        ...
        return res.status(500).json({ success: false, error: "Please Verify your Email" })
      }
...
router.post('/email/activation', async (req, res) => {
  try {
    ...
    if (user) {
      const updated = await User.findByIdAndUpdate(userId.user.id, { isVerified: true }).then(() => {
